...
class PieView(context: Context?, attrs: AttributeSet?) : View(context, attrs) {
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        for ((index, angle) in ANGLES.withIndex()) {
            ...
            if (index == 1) {
                canvas?.save()
                canvas?.translate(OFFSET_LENGTH * cos(Math.toRadians(startAngle + angle / 2f.toDouble()))
                    .toFloat(), OFFSET_LENGTH * sin(Math.toRadians(startAngle + angle / 2f.toDouble())).toFloat())
            }
